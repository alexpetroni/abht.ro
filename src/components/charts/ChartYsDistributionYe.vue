<template>
  <div>
    ...
    <div class="row">
      <div class="col-sm-12">
        <h3>Distribuţie medie Ys funcţie de Ye</h3>
      </div>
    </div>

    <div v-if="!ysDistributionYe.updated">
      Fetch data
    </div>

    <div v-if="ysDistributionYe.updated && !ysDistributionYe.data.length">
      <no-data-chart></no-data-chart>
    </div>

    <div v-show="ysDistributionYe.updated && ysDistributionYe.data.length">
      <div class="row">
...
export default{
  ...
  data() {
    return {
      colorMapCategory: [
        {  data_id: "1_traverse" , color: 'red', label: "Traverse", explain: "Ye = 0" },
        {  data_id: "2_praguri" , color: 'orange', label: "Praguri", explain: "0 < Ye <= 2"},
        {  data_id: "3_baraje" , color: 'yellow', label: "Baraje", explain: "Ye > 2"},
      ]
    }
  },

  methods: {
    ...mapActions(['fetchYsDistributionYeData']),
    ...
    showChart(){
      ...
      this.colorMapCategory.forEach(e => {
        ...
        let dataItem = this.ysDistributionYe.data.find( d => e.data_id == d._id  )
        if(dataItem){
  ...
  computed: {
    ...mapGetters([ 'ysDistributionYe' ]),

    chartData(){
      let cd = []

      let totalConstructions = this.ysDistributionYe.data.reduce(function(sum, elem){ return sum + elem.count }, 0)
  ...
  watch: {
    "ysDistributionYe": function(val){
      if(this.ysDistributionYe.updated){
        this.showChart()
      }
    }
  },
  ...
  created(){
    if(!this.ysDistributionYe.updated){
      console.log('request fetchYsDistributionYeData')
      this.fetchYsDistributionYeData()
    }
  },
